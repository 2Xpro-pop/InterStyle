<script lang="ts">
	...
	const otherLocale = (): Locale => (data.locale === 'ru' ? 'kg' : 'ru');
	const langSuffix = () => (data.locale === defaultLocale ? '' : `?lang=${data.locale}`);
...
<div class="site-shell">
	<header class="site-header">
		<div class="container nav-row">
			<a class="brand" href="/{langSuffix()}">InterStyle</a>
			<nav>
				<a href="/{langSuffix()}">{t(data.locale, 'nav.home')}</a>
				<a href="/curtains{langSuffix()}">{t(data.locale, 'nav.curtains')}</a>
				<a href="/reviews{langSuffix()}">{t(data.locale, 'nav.reviews')}</a>
			</nav>
			<div class="header-actions">
				<a href={switchLangHref()} class="locale-btn">{localeNames[otherLocale()]}</a>
				<div class="socials">
					<a href="https://wa.me/996550123456" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp">
					...
					<a href="https://2gis.kg/osh/search/InterStyle" target="_blank" rel="noopener noreferrer" aria-label="2ГИС">
					...
					<a href="https://www.instagram.com/interstyle.kg/" target="_blank" rel="noopener noreferrer" aria-label="Instagram">
